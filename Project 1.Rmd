...
However,Mondial has inconsistent number of nodes and/or subnodes throughout the XML content structure. It seems that we can't use standard protocols to fix those. We need to do some ad hoc commands to make that work. As a result we are switching to another more structured dataset called orders from the same repository, which we had success transforming the XML. See code below. We commneted out the lines for Mondial in case we stil want to challenge ourselves later.
...
## Aggregation Summary
Using the Order date and the newly created season varible to aggregate some data. Mean and Median by Year, standard deviation by season.
```{r Summaries}
...
## Bar Plot
```{r}
bar <- ggplot(orderTB_copy, aes(x = Year))
bar + geom_bar(aes(fill = Season)) + labs(x = "Year", y = "Transaction Counts", title = "Transaction Record from 1992 - 1998")
```
...
```
The above plot is too crowded to see the pattern. Do a line plot using sumary data instead.
...
```{r}
scatter2 <- ggplot(orderTB_copy, aes(x = month(O_ORDERDATE), y = O_TOTALPRICE)) 
scatter2 + geom_point(aes(col = Season)) + facet_wrap(~ Year) + labs(x = "Month", y = "Total Order Price per Customer", color = "Seasons")
```
