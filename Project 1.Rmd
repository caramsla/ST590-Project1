...
```{r message=FALSE}
#We noticed some issues in using a URL with an https epithet. We had to load the URL using RCurl and then change the security settings.
...
#Rename variables to get rid of collection.poi beginning portion
names(ajaxTBwide)<-sub("\\w{10}","",names(ajaxTBwide))
names(ajaxTBwide)<-sub(".","",names(ajaxTBwide))
names(ajaxTBwide)<-sub("\\w{3}","",names(ajaxTBwide))
names(ajaxTBwide)<-sub(".","",names(ajaxTBwide))
```
