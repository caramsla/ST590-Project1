...
#XML: What is is and why should we use it? <br>
...
There are several ways and packages that could be used to read in XML data.  The 'XML' package has many online tutorials, help, and trouble shooting available.  This, along with the fact that it contains many functions that make it easy to access and convert your data making it our recommendation for reading in XML data.
...
#### Aggregation Summary
Use the order date and the newly created season variable to aggregate some data; mean and median by year, and standard deviation by season.
```{r Summaries}
...
```{r Box2}
b2 <- ggplot(orderTB_copy, aes(x = as.factor(Year), y = O_TOTALPRICE, fill = Season))  + geom_boxplot() + labs(x = "Year", y = "Total Order Price per Customer", title = "Boxplot of Total Order Price by Year and Season") + facet_wrap(~Year, ncol = 3, scale = "free")
b2

# I added a facet wrap.  I don't think it helped the graph be any less busy.  Try it both ways and see what we like?! - Mandy
# B: I think box plots are not that crowded and puting them together enables user to see them holistically and compare year to year. I am fine with either ways.
```
...
# B: I think we can keep these as table() functions are good at showing frequency counts (like proc freq in SAS). This gives a different flavor from the dplyr::groupby and dplyr::summarise, which are used to calculate other aggregates.
...
#### Custom Function
A function that finds a customer and how much they have spent with the company and the date range in which they did it.
```{r custom}
# I need to add some logic and verbage when an invalid customer ID is entered
...
getTotalperCust<-function(df, variable){
  ...
  if(!(variable%in%df$O_CUSTKEY)){
    stop("Customer does not exist,please try with a valid customerKey")
  }
