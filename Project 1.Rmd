...
#Group A Signing on...
```{r message=FALSE}
#We noticed some issues in using a URL with a https epithet so had to load the URL using rCurl and then change the security settings
...
```
Getting there, but now the entire dataset is in a single line so we'll have to figure out how to change the rows into columns but only do so for every 24 rows.  
